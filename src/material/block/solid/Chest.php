...
class ChestBlock extends SolidBlock{
	...
	public function place(Item $item, Player $player, Block $block, Block $target, $face, $fx, $fy, $fz){
		if($block->inWorld === true){
			$block0 = $this->level->getBlockFace($block, 2);
			$block1 = $this->level->getBlockFace($block, 3);
			$block2 = $this->level->getBlockFace($block, 4);
			$block3 = $this->level->getBlockFace($block, 5);
			if($block0->getID() !== CHEST and $block1->getID() !== CHEST and $block2->getID() !== CHEST and $block3->getID() !== CHEST){
				...
				$this->level->setBlock($block, $this->id, $faces[$player->entity->getDirection()]);
